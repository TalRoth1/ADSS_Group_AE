...
public class ProductFacadeTest {
    ...
    public void testUpdateNonExistentProduct() {
        assertThrows(RuntimeException.class,
                () -> productFacade.updateProduct(0, "Fake product", 0.0, 0.0, 0, 123, new String[] { "Fake" }));
    }
    ...
    public void testAddProductAndItemAndUpdateItem() {
        int branchId = branchFacade.addBranch("TestBranch", "TestAddress");
        int productId = productFacade.addProduct("Tnuva milk", 0.7, 1.0, 0, 1, new String[] { "Milk" });
        int itemId = productFacade.addItem(productId, "Milk bottle", false, new Date(), branchId,
                new String[] { "Shelf A1" });
        assertDoesNotThrow(() -> productFacade.updateItem(itemId, null, false, branchId, new String[] { "Shelf B5" }));
    }
    ...
    public void testAddProductAndItemAndUpdateItem2() {
        ...
        int itemId = productFacade.addItem(productId, "Milk Bottle", false, new Date(), branchId,
                new String[] { "C1" });
        assertDoesNotThrow(
                () -> productFacade.updateItem(itemId, "Milk Bottle Large", true, branchId, new String[] { "C2" }));
    }
    ...
    public void testDefectedReportIsNotNull() {
        ...
        int itemId = productFacade.addItem(productId, "Milk Bottle", false, new Date(), branchId,
                new String[] { "C1" });
        productFacade.updateItem(itemId, null, false, branchId, null);
